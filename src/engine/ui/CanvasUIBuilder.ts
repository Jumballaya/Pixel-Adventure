...
export class CanvasUIBuilder {
  ...
  private getAttributesFromDomElement($el: HTMLElement) {
    ...
    const backgroundColor = $el.getAttribute('backgroundColor') || 'rgba(0,0,0,0)';
    ...
    const content = Array.from($el.childNodes).reduce((acc, cur) => {
  ...
  private setupElement(el: CanvasElement, attributes: Record<string, any>): CanvasElement {
    el.width = attributes.width;
    el.height = attributes.height;
    el.position = attributes.position;
    el.visible = attributes.visible;
    el.disabled =  attributes.disabled === null ? false : attributes.disabled === 'true';
  ...
  private createElement($el: HTMLElement, attributes: Record<string, any>): CanvasElement {
    if (this.domElementIsTextType($el)) {
      return this.setupElement(new CanvasText(attributes.content, attributes.position), attributes);
    }
    if (this.domElementIsButtonType($el)) {
      return this.setupElement(new CanvasButton(attributes.width, attributes.height, attributes.content, attributes.position), attributes);
    }
    return this.setupElement(new CanvasElement(
      attributes.width,
      attributes.height,
      attributes.position,
      new Sprite(attributes.backgroundImage || attributes.backgroundColor, undefined, undefined, attributes.scale)
    ), attributes);
  }
